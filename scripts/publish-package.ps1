#!/usr/bin/env pwsh
# Cross-platform script to publish ApiPipeline.NET package to GitHub Packages
...
Write-Host "Publishing ApiPipeline.NET v$Version to GitHub Packages" -ForegroundColor Green
Write-Host ""
...
if ($versionCheck.Exists -eq $true) {
    ...
    if (-not $deleted) {
        ...
        Write-Host "  2. Select ApiPipeline.NET, delete version $actualVersion, then run this script again" -ForegroundColor Yellow
        exit 1
    }
...
# Step 6: Push symbols package if present
$SymbolsFile = Join-Path $PackageOutput "$PackageName.$actualVersion.snupkg"
if (Test-Path $SymbolsFile) {
    Write-Host ""
    Write-Host "Step 6: Publishing symbols package..." -ForegroundColor Yellow
